...
#ver 0.7
import os #предназначен для взаимодействия программы с операционной системой
from random import randint #Возвращает случайное число в пределах заданного промежутка
from random import choice #Возвращает случайный элемент заданной последовательности

#класс object главный супер класс


class FieldPart(object): #класс для выбора поле игры 
    main = 'map' 		#карта на которой отображаются наши корабли  
    radar = 'radar'		#карта на которой расположены корабли противника 	
    weight = 'weight'	#область с тестовыми подсказками 
...
def draw_field(self, element):
        ...
        board = self.get_field_part(element)
...
class FieldPart(object): #класс для выбора облости отрисовки 
    main = 'map' 		#карта на которой отображаются наши корабли  
    radar = 'radar'		#карта на которой расположены корабли противника 	
    weight = 'weight'	#область с тестовыми подсказками 
							
class Cell(object):# тут мы задаем визуальное отображение клеток
	ship_cell = '■'
	empty_cell = 'O'
	destroyed_ship = 'X'
	miss_cell = 'T'  
...
class Game(object):
    letters = ("1", "2", "3", "4", "5", "6")# опишем буквы используемые для отображения координат
	ships_rules = [3, 2, 2, 1, 1, 1, 1] #какие корабли игрок должен выставить до начала боя
	field_size = len(letters)#величина поля
	
	def __init__(self):  #методом инициализации экземпляра этого класса
        self.players = [] #Игра будет хранить игроков в cписке
        self.current_player = None # текуший игрок 
        self.next_player = None # следущий игрок 
        self.status = 'prepare' #prepare, in_game, game_over (для отображения игры)
		
	def start_game(self):#просто объявляем текущего игрока и следующего
        self.current_player = self.players[0]
        self.next_player = self.players[1]

    def status_check(self):# функция переключения статуса игры
 ...
 def ships_setup(self, player):#Передаём в функцию ссылку на игрока и игра просит его расставить корабли или расставляет автоматически
        ...
        for ship_size in Game.ships_rules:
            ...
            retry_count = 10
            ship = Ship(ship_size, 0, 0, 0)# создаем предварительно экземпляр коробля просто нужно для задачи размера
			
            while True:

                Game.clear_screen()
                if player.auto_ship_setup is not True: #not (x is True)
...
class Player(object):
      ...
      def __init__(self, name, is_ai, skill):
        self.name = name  #име 
        self.is_ai = is_ai #является игрок ИИ
        self.auto_ship_setup = True #автоматически расставлять корабли для игрока или вручную
        self.skill = skill #относится к ИИ
        self.message = [] #сообщение об игроке 
        self.ships = []  #наполняется имеющимися у игрока кораблями
        self.enemy_ships = [] #какие корабли остались у противника
        self.field = None  #поле игрока 
    ...
    def get_input(self, input_type):
        ...
        if input_type == "ship_setup":
            ...
            if self.is_ai or self.auto_ship_setup: #растоновка кораблей для ИИ в авто режиме
            ...
            if x not in Game.letters or not y.isdigit() or int(y) not in range(1, Game.field_size + 1) or #проверка на не правельный ввод данных 
                    r not in ("H", "V"):
...
class Ship:
    ...
    def __init__(self, size, x, y, rotation):
        ...
        self.hp = size #от размера зависет здоровье коробля 
        self.x = x #координаты начала корабля по х
        self.y = y #координаты начала корабля по у
...
if __name__ == '__main__':
    ...
    players = []#указываем значение с именами игроков пустым 
    ...
    while True:
        ...
        if game.status == 'in game':
            ...
            # если промах - передаем ход следующему игроку.
            if shot_result == 'miss':
